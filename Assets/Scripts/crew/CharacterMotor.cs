...
public class CharacterMotor : MonoBehaviour
{
    ...
    public class CharacterMotorMovement
    {
		...
		public float maxGravityAirAcceleration = 20.0f;
		public float maxNoGravityAirAcceleration = 20.0f;
	...
	private void ApplyGroundedUpdate(float pushDownOffset){
    ...
    private void UpdateFunction()
    {
        ...
        if(movement.velocity.y < velocity.y - 0.005)
        {
		...
		ApplyGroundedUpdate(pushDownOffset);
    ...
    private Vector3 ApplyInputVelocityChange(Vector3 velocity)
    {
        ...
        if(grounded && TooSteep())
        {
            ...
            var projectedMoveDir = Vector3.Project(input.direction, desiredVelocity);
            // Add the sliding direction, the spped control, and the sideways control vectors
            desiredVelocity = desiredVelocity + projectedMoveDir * sliding.speedControl + (input.direction - projectedMoveDir) * sliding.sidewaysControl;
    ...
    private Vector3 ApplyGravityAndJumping(Vector3 velocity)
    {
        ...
        else
        {
            ...
            if(jumping.jumping && jumping.holdingJumpButton)
            {
                ...
                if(Time.time < jumping.lastStartTime + jumping.maxHeight / CalculateJumpVerticalSpeed(jumping.minHeight))
                {
        ...
        if(grounded)
        {
            ...
            if(jumping.enabled && isInputEnabled && (Time.time - jumping.lastButtonDownTime < 0.2))
            {
                grounded = false;
                ...
                velocity.y = 0;
                velocity += jumping.jumpDir * CalculateJumpVerticalSpeed(jumping.minHeight);
    ...
    {
        // Find desired velocity
        Vector3 desiredLocalDirection = tr.InverseTransformDirection(input.direction);
